/**
 * cargar-exito-2025-2026.mjs
 * Carga BASE BORDEN COLOMBIA 2025 A 06-03-26.xlsx a fact_ventas_exito.
 * Convierte COP → USD usando TRM diaria de Banrep
 * (Tasa de cambio del peso colombiano.csv en Downloads).
 * Match por codigo_barras contra dim_producto (no la colombia).
 */
import pg from 'pg'
...
const TRM = new Map()
{
  ...
  for await (const raw of rl) {
    ...
    // Formato: "1991/11/27";693,32 — TRM con coma decimal
    const parts = line.split(';')
    if (parts.length < 2) continue
    const date = parts[0].replace(/"/g, '').trim()  // YYYY/MM/DD
...
console.log(`   ${TRM.size.toLocaleString()} fechas con TRM`)
// Helper: si no hay TRM ese día, buscar el más cercano hacia atrás
...
for (let i = 1; i < xrows.length; i++) {
  ...
  const catRaw   = String(r[4] ?? '').trim()
  const codPdv   = String(r[5] ?? '').trim()
  const puntoVta = String(r[6] ?? '').trim()
  const plu      = String(r[7] ?? '').trim()
  const cbRaw    = String(r[8] ?? '').trim()
  const desc     = String(r[9] ?? '').trim()
  const ano      = Number(r[10])
  const mes      = Number(r[11])
  const dia      = Number(r[12])
  const und      = Number(r[13]) || 0
  const cop      = Number(r[14]) || 0
...
if (sinUpcs.size) console.log(`   UPCs sin match (${sinUpcs.size}): ${[...sinUpcs].slice(0, 15).join(', ')}`)
if (sinTrmDates.size) console.log(`   Fechas sin TRM (${sinTrmDates.size}): ${[...sinTrmDates].slice(0, 5).join(', ')}`)

// ── 4. Aggregate por (fecha, punto_venta, codigo_barras) ──────────────────
...
console.log(`   💰 a insertar: ${sumUnd.toLocaleString()} und · COP ${sumCOP.toLocaleString('es-CO', {maximumFractionDigits:0})} · $${sumUSD.toLocaleString('en-US', {maximumFractionDigits:2})}`)

// ── 5. DELETE + INSERT ────────────────────────────────────────────────────
console.log('\n🗑️  Borrando fact_ventas_exito (pais=CO, ano IN 2025/2026)…')
const del = await pool.query(`DELETE FROM fact_ventas_exito WHERE pais='CO' AND ano IN (2025, 2026)`)
...
// ── 7. Verificación ───────────────────────────────────────────────────────
console.log('\n🔎 fact_ventas_exito por año y mes:')
...
await pool.end()
console.log('\n🎉 Carga GRUPO ÉXITO Colombia completa')
