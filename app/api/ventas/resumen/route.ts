...
export async function GET(req: NextRequest) {
  try {
    ...
    const cacheKey = `resumen-v12:${new URL(req.url).searchParams.toString()}`
    const { data: result } = await withCache(
      ...
      async () => {
        ...
        const [kpiQ, timeQ, catQ, paisQ, skuQ, subcatQ, clienteQ, diaSemanasR] = await Promise.all([
          ...
          // skuQ: agrupar por (sku, descripcion) — combina el mismo producto con múltiples barcodes
          //        pero mantiene separados productos distintos que comparten SKU (ej: helados SKU 1234)
          //        barcode canónico de dim_producto; fallback al mínimo del dato de venta
          pool.query(
            `SELECT m.sku,
                    m.descripcion,
                    MIN(m.categoria) AS categoria,
                    COALESCE(MIN(p.codigo_barras), MIN(m.codigo_barras)) AS codigo_barras,
             ...
             FROM (SELECT sku, descripcion, categoria, codigo_barras, ventas_valor, ventas_unidades
                   ...
                   WHERE ${where} AND sku IS NOT NULL AND sku != '') m
             LEFT JOIN dim_producto p USING (sku)
             GROUP BY m.sku, m.descripcion
             ORDER BY ventas_valor DESC LIMIT 10`, params),
          pool.query(
