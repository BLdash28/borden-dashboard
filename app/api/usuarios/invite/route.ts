...
export async function POST(req: NextRequest) {
  try {
    ...
    const { error: mailErr } = await resend.emails.send({
      from,
      to:      [email],
      subject: 'Te invitaron al BL Food Dashboard',
      html: `<!DOCTYPE html>
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f3f4f6;padding:32px 16px;margin:0;">
  <div style="max-width:560px;margin:0 auto;">
    ...
    <div style="background:#fff;border-radius:12px;padding:28px;border:1px solid #e5e7eb;">
      <h2 style="margin:0 0 8px;font-size:18px;color:#111827;">¡Bienvenido${full_name ? `, ${full_name}` : ''}!</h2>
      <p style="color:#6b7280;font-size:14px;margin:0 0 24px;">
        Fuiste invitado al dashboard de análisis de BL Food. Haz clic en el botón de abajo para crear tu contraseña y acceder.
      </p>
      ...
      <p style="color:#9ca3af;font-size:12px;margin:0;">
        Este enlace expira en 24 horas. Si no esperabas esta invitación, ignora este email.
      </p>
    </div>
  </div>
</body></html>`,
