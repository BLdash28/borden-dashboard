  ...
  try {
    ...
    const [ventaR, invR, reordenR, colaR] = await Promise.all([
      ...
      pool.query(`
        ...
        FROM mmv_sellout_mensual WHERE ano = ${ano}
      `),
      ...
      pool.query(`
        WITH venta AS (
          ...
          FROM mmv_sellout_mensual WHERE ano IN (2025,2026) GROUP BY sku
        )
        SELECT COUNT(*) AS cnt
      ...
      pool.query(`
        WITH v AS (
          SELECT sku, SUM(ventas_valor) AS valor FROM mmv_sellout_mensual
