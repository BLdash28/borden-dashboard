...
export async function GET(req: NextRequest) {
  ...
  try {
    const cacheKey = `recomendaciones:${searchParams.get('paises')}:${searchParams.get('categorias')}:${searchParams.get('anos')}:${searchParams.get('meses')}:${restrictions?.isRestricted}`
    ...
    const { data, cached } = await withCache(cacheKey, async () => {
      ...
      if (perQ.rows.length === 0) {
        return { skus: [], recientes: [], anteriores: [], periodoRef: null }
      }
