...
export default function Distribucion75() {
  ...
  const descargarCSV = () => {
    const csv = ['#,SKU,Descripción,Categoría,Valor,% Acumulado,Top 75%',
      ...display.map(r => `${r.rank},"${r.sku}","${r.descripcion}","${r.categoria}",${r.valor.toFixed(2)},${r.pct_acum}%,${r.es_top75?'Sí':'No'}`)
    ].join('\n')
      ...
      {resumen && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            { label: '% de SKUs',    value: resumen.pct_skus.toFixed(1) + '%' },
            { label: 'Venta Total',  value: fmt(resumen.valor_total) },
          ].map(k => (
