...
interface CediRow {
  upc: string; codigo_barras: string | null; sku: string | null; descripcion: string | null
  categoria: string | null; subcategoria: string | null
  pais: string; inv_mano_cajas: number; inv_orden_cajas: number; fecha: string
}

type MainSortKey   = 'inv_mano' | 'tiendas' | 'venta_dia' | 'doh' | 'semaforo'
type CediSortKey   = 'inv_mano_cajas' | 'inv_orden_cajas' | 'pais'
...
const DEFAULT_CEDI_SORT: { key: CediSortKey; dir: SortDir } = { key: 'inv_mano_cajas', dir: 'desc' }
  ...
  return [...arr].sort((a, b) => {
    ...
    if (key === 'semaforo') {
      ...
      return dir === 'asc'
        ...
        : String(b.pais).localeCompare(String(a.pais))
    } else {
...
export default function InventarioTiendas() {
  const [tab,      setTab]      = useState<'tiendas' | 'cedi'>('tiendas')
  const [paises,   setPaises]   = useState<string[]>([])
  const [cats,     setCats]     = useState<string[]>([])
  const [filtro,   setFiltro]   = useState<string>('todos')
  const [rows,     setRows]     = useState<Row[]>([])
  const [cediRows, setCediRows] = useState<CediRow[]>([])
  const [loading,  setLoading]  = useState(true)
  const [detalle,  setDetalle]  = useState<Detalle | null>(null)
  const [loadDet,  setLoadDet]  = useState(false)

  const [mainSort, setMainSort] = useState<{ key: MainSortKey; dir: SortDir }>(DEFAULT_SORT)
  const [cediSort, setCediSort] = useState<{ key: CediSortKey; dir: SortDir }>(DEFAULT_CEDI_SORT)
  ...
  const toggleCediSort = (key: string) => {
    setCediSort(prev => {
      if (prev.key !== key) return { key: key as CediSortKey, dir: 'desc' }
      if (prev.dir === 'desc') return { key: key as CediSortKey, dir: 'asc' }
      return DEFAULT_CEDI_SORT
    })
  }
  ...
  const cargar = useCallback(async (ps: string[], cs: string[]) => {
    setLoading(true)
    setMainSort(DEFAULT_SORT)
    setCediSort(DEFAULT_CEDI_SORT)
    try {
      const qs = new URLSearchParams()
      if (ps.length) qs.set('pais', ps.join(','))
      if (cs.length) qs.set('categoria', cs.join(','))
      const [resT, resC] = await Promise.all([
        fetch('/api/comercial/ejecucion/inventario?' + qs),
        fetch('/api/comercial/ejecucion/inventario/cedi?' + qs),
      ])
      if (resT.ok) { const j = await resT.json(); setRows(j.rows ?? []) }
      if (resC.ok) { const j = await resC.json(); setCediRows(j.rows ?? []) }
    } catch { setRows([]); setCediRows([]) } finally { setLoading(false) }
  }, [])
  ...
  const cediDisplay = sortRows(cediRows, cediSort.key, cediSort.dir)
    ...
    if (tab === 'tiendas') {
      ...
      const csv = [
        ...
        ...cediDisplay.map(r =>
          ...
          `"${r.pais}",${r.inv_mano_cajas},${r.inv_orden_cajas}`
        )
      ].join('\n')
      const a = document.createElement('a')
      a.href = URL.createObjectURL(new Blob(['﻿'+csv], { type: 'text/csv;charset=utf-8;' }))
      a.download = `inventario_cedi_${new Date().toISOString().slice(0,10)}.csv`
      a.click()
    }
  }
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">
            {tab === 'tiendas'
              ? 'Días de cobertura (DOH) por producto en tiendas Walmart'
      ...
      {tab === 'cedi' && (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-x-auto">
          {loading
            ? <div className="h-40 flex items-center justify-center text-gray-300 text-sm">Cargando...</div>
            : cediDisplay.length === 0
              ? <div className="h-40 flex items-center justify-center text-gray-400 text-sm">Sin datos disponibles.</div>
              : (
                <table className="w-full text-xs min-w-[700px]">
                  <thead className="bg-gray-50 border-b border-gray-200">
                    <tr className={thBase}>
                      <th className="text-left py-3 px-4">Código / Descripción</th>
                      <th className="text-left py-3 px-3">Categoría</th>
                      <th className="text-left py-3 px-3">Subcategoría</th>
                      <SortTh label="País"          colKey="pais"            sort={cediSort} onSort={toggleCediSort} className="text-left" />
                      <SortTh label="Inv. Mano (cj)" colKey="inv_mano_cajas"  sort={cediSort} onSort={toggleCediSort} className="text-right" />
                      <SortTh label="En Orden (cj)"  colKey="inv_orden_cajas" sort={cediSort} onSort={toggleCediSort} className="text-right" />
                    </tr>
                  </thead>
                  <tbody>
                    {cediDisplay.map((r, i) => (
                      <tr key={`${r.upc}-${r.pais}`}
                        ...
                        <td className="py-2 px-3 text-gray-500">{r.categoria ?? <span className="text-gray-300">—</span>}</td>
                        <td className="py-2 px-3 text-gray-400 text-[11px]">{r.subcategoria ?? <span className="text-gray-300">—</span>}</td>
                        <td className="py-2 px-3 text-gray-600 font-medium">{r.pais}</td>
                        <td className="py-2 px-3 text-right font-bold text-gray-800">{fmtN(r.inv_mano_cajas)}</td>
                        <td className="py-2 px-3 text-right text-gray-500">
                          {r.inv_orden_cajas > 0
                            ? fmtN(r.inv_orden_cajas)
      ...
      {/* Panel lateral — tiendas */}
      {detalle && (
        <>
          <div className="fixed inset-0 bg-black/20 z-40" onClick={() => setDetalle(null)} />
