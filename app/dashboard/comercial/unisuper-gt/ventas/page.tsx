...
type SortKey = 'venta_neta' | 'unidades' | 'nombre_sucursal' | 'descripcion_sku' | 'categoria'
...
interface Row {
  ...
  categoria: string; subcategoria: string; codigo_sku: string
  descripcion_sku: string; unidades: number; venta_neta: number
}
interface Kpis {
  total_venta: number; total_unidades: number; sucursales: number; skus: number; fecha: string
}

function sortRows(arr: Row[], key: SortKey, dir: SortDir): Row[] {
  return [...arr].sort((a, b) => {
    if (key === 'nombre_sucursal' || key === 'descripcion_sku' || key === 'categoria') {
...
export default function UnisuporVentas() {
  ...
  const [sort,      setSort]      = useState<{ key: SortKey; dir: SortDir }>({ key: 'venta_neta', dir: 'desc' })
  ...
  const toggleSort = (k: string) => {
    setSort(prev => {
      ...
      return { key: 'venta_neta', dir: 'desc' }
    })
  }
  ...
  const cargar = useCallback(async (cs: string[], ss: string[], f: string) => {
    ...
    try {
      ...
      if (f)         qs.set('fecha',     f)
      const res = await fetch('/api/comercial/unisuper-gt/ventas?' + qs)
  ...
  const exportCSV = async () => {
    ...
    try {
      ...
      a.download = `unisuper_ventas_${fecha || 'latest'}.csv`
      a.click()
              ...
              <table className="w-full text-xs min-w-[700px]">
                <thead>
                  <tr className={`${thBase} border-b border-gray-100`}>
                    ...
                    <SortTh label="Unidades"    colKey="unidades"        sort={sort} onSort={toggleSort} className={thBase + ' text-right'} />
                    <SortTh label="Venta Neta"  colKey="venta_neta"      sort={sort} onSort={toggleSort} className={thBase + ' text-right pr-4'} />
                  </tr>
                </thead>
                <tbody>
                  {display.map((r, i) => (
                    <tr key={`${r.codigo_sucursal}-${r.codigo_sku}`}
                      ...
                      <td className="py-2.5 px-3 text-right text-gray-700">{fmtN(r.unidades)}</td>
                      <td className="py-2.5 px-3 pr-4 text-right font-semibold text-gray-800">{fmtQ(r.venta_neta)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
