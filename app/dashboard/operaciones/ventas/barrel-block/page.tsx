...
export default function BarrelBlockPage() {
  ...
  useEffect(() => {
    fetch('/api/operaciones/ventas/barrel-block?tipo=filtros')
      .then(r => r.json())
      .then(j => {
        setSkus(j.skus || [])
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <h1 className="text-xl font-bold" style={{ color: 'var(--t1)' }}>Leche &amp; Crema</h1>
            ...
            <div className="px-6 py-5 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                ...
                <Field label="Proveedor">
                  <div className={`${inp} opacity-60 truncate`} style={{ ...inputStyle, cursor: 'default' }}>DFA</div>
                </Field>
              </div>
              <Field label="SKU *">
      ...
      <div className="card p-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {[
            { label: 'SKU', content: (
              <select value={fSku} onChange={e => setFSku(e.target.value)} className={sel} style={selectStyle}>
                <option value="Todos">Todos</option>
                {skus.map(s => <option key={s.sku} value={s.sku}>{s.descripcion || s.sku}</option>)}
              </select>
            )},
            { label: 'Año', content: (
              <select value={fAno} onChange={e => setFAno(e.target.value)} className={sel} style={selectStyle}>
                <option value=''>Todos</option>
                {anos.map(a => <option key={a} value={a}>{a}</option>)}
              </select>
            )},
            { label: 'Mes', content: (
              <select value={fMes} onChange={e => setFMes(e.target.value)} className={sel} style={selectStyle}>
                <option value=''>Todos</option>
      ...
      {empty && (
        <div className="card p-12 text-center">
          ...
          <p className="font-medium text-[14px]" style={{ color: 'var(--t2)' }}>No hay compras registradas</p>
        ...
        <>
          ...
          <div className="card overflow-hidden">
            <div className="px-5 py-4 border-b flex items-center justify-between" style={{ borderColor: 'var(--border)' }}>
              <div className="font-semibold text-[13px]" style={{ color: 'var(--t1)' }}>Historial de compras</div>
                  ...
                  {loading
                    ...
                    : compras.map(c => (
                        <tr key={c.id} className="border-b transition-colors hover:bg-white/5"
                          ...
                          <td className="px-4 py-3" style={{ color: 'var(--t3)' }}>{c.proveedor}</td>
          ...
          <div className="rounded-xl px-4 py-3 text-[12px] border"
            ...
            El sistema usa el costo más reciente de Leche &amp; Crema como fuente primaria para calcular la Ganancia %.
          </div>
        </>
      )}
    </div>
