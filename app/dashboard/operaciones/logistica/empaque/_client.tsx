...
const SKUS_EMPAQUE: Record<string, { desc: string; u_m: string }> = {
  '0745210597001': { desc: 'BORDEN LECHE ENTERA 1000 ML',             u_m: 'UNIDAD' },
  '0745210597002': { desc: 'BORDEN LECHE SEMIDRESCRE 1000 ML',        u_m: 'UNIDAD' },
  '0745210597003': { desc: 'BORDEN LECHE DESCREMADA 1000 ML',         u_m: 'UNIDAD' },
  '0745210597004': { desc: 'BORDEN LECHE DESCREMADA DESLAC 1000 ML',  u_m: 'UNIDAD' },
  '0745210597005': { desc: 'BORDEN LECHE SEMIDESCRE DESLAC 1000 ML',  u_m: 'UNIDAD' },
  '0745210597006': { desc: 'BORDEN 3 PK LECHE ENTERA 3000 ML',        u_m: 'PACK'   },
  '0745210597007': { desc: 'BORDEN 3 PK LECHE SEMIDRESCRE 3000 ML',   u_m: 'PACK'   },
  '0745210597008': { desc: 'BORDEN 3 PK LECHE DESCREMADA 3000 ML',    u_m: 'PACK'   },
  '0745210597009': { desc: 'BORDEN 3 PK LECHE DES DESLAC 3000 ML',    u_m: 'PACK'   },
  '0745210597010': { desc: 'BORDEN 3PK LECHE SEMI DESLAC 3000 ML',    u_m: 'PACK'   },
  '0745210597013': { desc: 'BORDEN 12 PK LECHE DESCREMADA 12000 ML',  u_m: 'PACK'   },
  '0745210597014': { desc: 'BORDEN 12 PK LECHE SEMIDRESCRE 12000 ML', u_m: 'PACK'   },
  '0745210597032': { desc: 'BORDEN 12 PK LECHE SEMI DESLAC 12000 ML', u_m: 'PACK'   },
  '0745210597099': { desc: 'BORDEN LECHE SEMIDESCREMADA 1000 ML',      u_m: 'UNIDAD' },
}

const MERMA_PCT = 0.025
...
export default function InvEmpaqueClient({ isAdmin }: { isAdmin: boolean }) {
  ...
  const [fEntrada, setFEntrada] = useState({
    fecha: '', cod_interno: '', descripcion: '', u_m: 'UNIDAD', cantidad: '', observacion: ''
  })
  ...
  const getMerma = (cant: number) => cant * MERMA_PCT
  const getDisp  = (cant: number) => cant - getMerma(cant)

  const displayRows = invRows.length > 0
    ? invRows
    : Object.entries(SKUS_EMPAQUE).map(([cod, { desc, u_m }]) => ({
        cod_interno: cod, descripcion: desc, u_m, cant_bodega: 0
      }))
  ...
  const submitEntrada = async () => {
    ...
    try {
      ...
      setFEntrada({ fecha:'', cod_interno:'', descripcion:'', u_m:'UNIDAD', cantidad:'', observacion:'' })
      cargar('inventario')
    } catch (e: any) { notify(e.message, true) }
  ...
  const submitSalida = async () => {
    ...
    try {
      ...
      const last = await post({ accion: 'ultimo_pedido_cod', cod_interno: fSalida.cod_interno })
        .catch(() => ({ id: null }))
      if (last?.id) await post({ accion: 'aprobar_pedido', pedido_id: last.id })
  ...
  const toggleSku = (cod: string) => {
    ...
    setPedSkus(next)
    if (!next.includes(cod)) { const c = { ...pedCants }; delete c[cod]; setPedCants(c) }
    setPedError('')
  }
  ...
  const submitPedido = async () => {
    ...
    setSaving(true)
    try {
      const ref   = pedRef || `PED-EMP-${Date.now()}`
      ...
      for (const cod of pedSkus) {
        ...
        const r = await post({ accion: 'ultimo_pedido_cod', cod_interno: cod }).catch(() => ({ id: null }))
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-800">Inventario Materiales Empaque</h1>
        </div>
      ...
      {tab === 'inventario' && (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <h3 className="font-semibold text-gray-700">Stock Actual — Materiales de Empaque</h3>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-xs text-gray-400 uppercase tracking-widest bg-gray-50 border-b border-gray-100">
                      ...
                      <th className="text-right px-4 py-3">MERMA (2.5%)</th>
                      ...
                      return (
                        <tr key={i}
                          ...
                          <td className="px-5 py-3">
                            ...
                            <span className="ml-2 text-[9px] bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded-full font-medium">EMP</span>
                          </td>
                          ...
                          <td className="px-4 py-3 text-right text-xs">
                            ...
                            <span className="text-gray-300 ml-1">(2.5%)</span>
                          </td>
                        ...
                        <tr key={i} className="border-b border-gray-50 hover:bg-gray-50">
                          ...
                          <td className="px-5 py-3 text-center">
                            <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${
                              r.estado === 'PROCESADO' ? 'bg-emerald-100 text-emerald-700' :
                      ...
                      {salRows.map((r, i) => (
                        <tr key={i} className="border-b border-gray-50 hover:bg-gray-50">
                          ...
                          <td className="px-5 py-3 text-gray-500 text-xs">
                            {r.ref_orden || (r.referencia_pedido ? `#${r.referencia_pedido}` : '—')}
                          </td>
                        </tr>
                      ))}
                    </tbody>
          ...
          <div className="space-y-4">
            ...
            <Field label="COD_INTERNO *">
              <select value={fEntrada.cod_interno}
                onChange={e => {
                  const info = SKUS_EMPAQUE[e.target.value]
                  setFEntrada(p => ({...p, cod_interno: e.target.value, descripcion: info?.desc || '', u_m: info?.u_m || 'UNIDAD'}))
                }} className={inp}>
                <option value="">— Seleccionar SKU —</option>
                {Object.entries(SKUS_EMPAQUE).map(([cod, { desc }]) => (
          ...
          <div className="space-y-4">
            ...
            <Field label="COD_INTERNO *">
              <select value={fSalida.cod_interno}
                onChange={e => {
                  const info = SKUS_EMPAQUE[e.target.value]
                  setFSalida(p => ({...p, cod_interno: e.target.value, descripcion: info?.desc || '', u_m: info?.u_m || 'UNIDAD'}))
                }} className={inp}>
                <option value="">— Seleccionar SKU —</option>
                {Object.entries(SKUS_EMPAQUE).map(([cod, { desc }]) => (
                  <option key={cod} value={cod}>{cod} · {desc}</option>
            ...
            {fSalida.cod_interno && (
              <div className="rounded-lg px-3 py-2 text-xs bg-purple-50 border border-purple-200 text-purple-800">
                ...
                {fmtN(getDisp(Math.round(toNum(invMap.current[fSalida.cod_interno]?.cant_bodega))))} {fSalida.u_m}
              </div>
            )}
      ...
      {showPedido && (
        <Modal title="Entrada de Pedido" onClose={() => setShowPedido(false)}>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <Field label="Fecha">
                <input type="date" value={pedFecha} onChange={e => setPedFecha(e.target.value)} className={inp}/>
              </Field>
              <Field label="Referencia / Lote">
                <input value={pedRef} onChange={e => setPedRef(e.target.value)}
                  placeholder="PED-EMP-2026-001" className={inp}/>
              </Field>
            </div>
            <Field label={`Seleccionar SKUs (${pedSkus.length} seleccionados)`}>
                      ...
                      {sel && (
                        <div onClick={e => e.stopPropagation()} className="flex-shrink-0">
                          <input type="number" value={cant} min={1}
                            onChange={e => { setPedCants(p => ({ ...p, [cod]: toNum(e.target.value) })); setPedError('') }}
            ...
            {pedSkus.length > 0 && (
              <div className={`rounded-lg px-4 py-3 text-sm flex items-center justify-between ${pedValido ? 'bg-emerald-50 border border-emerald-200' : 'bg-red-50 border border-red-200'}`}>
                <span className="text-gray-600">Total unidades pedidas:</span>
                    ...
                    <table className="w-full text-xs">
                      ...
                      <tbody>
                        {historialRows.map((h, i) => {
                          ...
                          const esAjuste  = h.tipo === 'AJUSTE'
                          return (
                            <tr key={i} className="border-b border-gray-50 hover:bg-gray-50">
                              <td className="px-3 py-2 text-gray-500">{fmtD(h.fecha)}</td>
                              <td className="px-3 py-2">
                                <span className={`font-medium px-1.5 py-0.5 rounded-full text-[10px] ${
                                  esEntrada ? 'bg-emerald-100 text-emerald-700' :
                                  esAjuste  ? 'bg-amber-100 text-amber-700' :
                                              'bg-purple-100 text-purple-700'
                                }`}>{h.tipo}</span>
                              </td>
                              <td className={`px-3 py-2 text-right font-bold ${esEntrada || esAjuste ? 'text-emerald-600' : 'text-red-500'}`}>
                                {esEntrada || esAjuste ? '+' : '-'}{fmtN(h.cantidad)}
                              </td>
