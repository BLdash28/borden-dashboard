...
const SKUS_LECHE = ['DNI-S124-01','DNI-S122-01','DNI-S120-01','DNI-S123-01','DNI-S121-01']
const COD_TAPA   = 'A100000125'
const COD_CINTA  = 'A885695101'

const MIN_TOTAL = 79000
const MIN_SKU   = 15800

function distribuirPedido(skus: string[]): Record<string, number> {
  const n = skus.length
  if (n === 0) return {}
  const base  = MIN_SKU
  const total = Math.max(MIN_TOTAL, Math.ceil(MIN_TOTAL / (base * n)) * base * n)
  const extra = total - base * n
  const dist: Record<string, number> = {}
  skus.forEach((s, i) => { dist[s] = i === 0 ? base + extra : base })
  return dist
}

function calcTapas(totalLeches: number): number { return totalLeches }
function calcCinta(totalLeches: number): number { return (totalLeches / 18720) * 1.42 }
function cantValida(v: number) { return v >= MIN_SKU && v % MIN_SKU === 0 }
...
export default function InvMaterialesPage() {
  ...
  const [fEntrada, setFEntrada] = useState({
    ...
    cantidad: '', proveedor: 'CENTROLAC', observacion: ''
  })
  ...
  useEffect(() => {
    fetch('/api/me').then(r => r.json()).then(d => { if (d.isAdmin) setIsSuperadmin(true) }).catch(() => {})
  }, [])
  ...
  const openHistorial = async (cod: string, desc: string) => {
    ...
    try {
      const res = await fetch(`/api/operaciones/inv-materiales?vista=historial&cod=${cod}`)
  ...
  const cargar = useCallback(async (t: Tab) => {
    ...
    try {
      if (t === 'inventario') {
        const res = await fetch('/api/operaciones/inv-materiales?vista=inventario')
        ...
        const res = await fetch('/api/operaciones/inv-materiales?vista=salidas')
        if (!res.ok) throw new Error(`API error ${res.status}`)
        const r = await res.json()
        if (r.error) throw new Error(r.error)
        setSalRows(r.rows)
      } else if (t === 'pedidos') {
        const res = await fetch('/api/operaciones/inv-materiales?vista=pedidos')
        if (!res.ok) throw new Error(`API error ${res.status}`)
        const r = await res.json()
        if (r.error) throw new Error(r.error)
        setPedRows(r.rows ?? [])
  ...
  const post = async (body: any) => {
    const res = await fetch('/api/operaciones/inv-materiales', {
      method: 'POST',
  ...
  const submitEntrada = async () => {
    ...
    try {
      ...
      if (SKUS_LECHE.includes(fEntrada.cod_interno)) {
        const tapa = invMap.current[COD_TAPA]
        if (tapa) {
          await post({
            accion: 'crear_pedido', cod_interno: COD_TAPA, cantidad_pedida: cant,
            referencia: `AUTO-TAPA-${fEntrada.observacion || fEntrada.fecha}`,
            fecha: fEntrada.fecha || new Date().toISOString().slice(0, 10),
          })
        }
      }
      notify(`Ingreso registrado${SKUS_LECHE.includes(fEntrada.cod_interno) ? ` — ${fmtN(cant)} tapas descontadas automáticamente` : ''}`)
      setShowEntrada(false)
      setFEntrada({ fecha:'', cod_interno:'', descripcion:'', u_m:'UNIDAD', cantidad:'', proveedor:'CENTROLAC', observacion:'' })
      cargar('inventario')
    } catch (e: any) { notify(e.message, true) }
  ...
  const toggleSku = (cod: string) => {
    ...
    setPedSkus(next)
    setPedCants(distribuirPedido(next))
    setPedError('')
  }

  const updateCant = (cod: string, val: number) => {
    const next = { ...pedCants, [cod]: val }
    setPedCants(next)
    const total = pedSkus.reduce((s, c) => s + (next[c] || 0), 0)
    if (total < MIN_TOTAL)
      setPedError(`Total: ${fmtN(total)} — mínimo requerido: ${fmtN(MIN_TOTAL)}`)
    else if (!pedSkus.every(c => cantValida(next[c] || 0)))
      setPedError('Todas las cantidades deben ser múltiplo de 15,800')
    else setPedError('')
  }

  const totalLeches = pedSkus.filter(c => SKUS_LECHE.includes(c)).reduce((s, c) => s + (pedCants[c] || 0), 0)
  const tapasAuto   = calcTapas(totalLeches)
  const cintaAuto   = calcCinta(totalLeches)
  const pedTotal    = pedSkus.reduce((s, c) => s + (pedCants[c] || 0), 0)

  const pedValido = pedSkus.length > 0
    && pedTotal >= MIN_TOTAL
    && pedSkus.every(c => cantValida(pedCants[c] || 0))
    && pedSkus.every(c => (pedCants[c] || 0) <= toNum(invMap.current[c]?.total_disponible))

  const submitPedido = async () => {
    if (!pedValido) return
    setSaving(true)
    try {
      const ref   = pedRef || `PED-${Date.now()}`
      ...
      if (totalLeches > 0 && tapasAuto > 0) {
        await post({ accion: 'crear_pedido', cod_interno: COD_TAPA, cantidad_pedida: Math.ceil(tapasAuto), referencia: `AUTO-TAPA-${ref}`, fecha })
        const rt = await post({ accion: 'ultimo_pedido_cod', cod_interno: COD_TAPA })
        if (rt.id) await post({ accion: 'aprobar_pedido', pedido_id: rt.id })
      }

      if (totalLeches > 0 && cintaAuto > 0) {
        await post({ accion: 'crear_pedido', cod_interno: COD_CINTA, cantidad_pedida: Math.ceil(cintaAuto), referencia: `AUTO-CINTA-${ref}`, fecha })
        const rc = await post({ accion: 'ultimo_pedido_cod', cod_interno: COD_CINTA })
        if (rc.id) await post({ accion: 'aprobar_pedido', pedido_id: rc.id })
      }

      notify(`Pedido procesado — ${fmtN(pedTotal)} uds leche${totalLeches > 0 ? ` + ${Math.ceil(tapasAuto).toLocaleString()} tapas + ${Math.ceil(cintaAuto).toLocaleString()} rollos cinta` : ''}`)
      setShowPedido(false)
  ...
  const getMerma = (cod: string, cant_bodega: number) =>
    cod === COD_TAPA ? cant_bodega * 0.0125 : cant_bodega * 0.025
  const getDisp = (cod: string, cant_bodega: number) =>
    cant_bodega - getMerma(cod, cant_bodega)
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-800">Inventario Materiales de Empaques</h1>
        </div>
      ...
      {tab === 'inventario' && (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <h3 className="font-semibold text-gray-700">Stock Actual — Materiales de Empaques</h3>
            <div className="flex items-center gap-3">
              <span className="text-xs text-gray-400 italic">↑ Click en una fila para ver historial</span>
              <span className="text-xs text-gray-400">{invRows.length} productos</span>
            </div>
          </div>
          {loading
            ? <div className="h-40 flex items-center justify-center text-gray-300 text-sm">Cargando...</div>
            : invRows.length === 0
              ? <div className="h-40 flex items-center justify-center text-gray-400 text-sm">Sin registros — ejecuta el SQL primero</div>
              : <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {invRows.map((r, i) => {
                        const isTapa     = r.cod_interno === COD_TAPA
                        const isCinta    = r.cod_interno === COD_CINTA
                        const isLeche    = SKUS_LECHE.includes(r.cod_interno)
                        const mermaPct   = isTapa ? 1.25 : 2.5
                        const merma      = getMerma(r.cod_interno, toNum(r.cant_bodega))
                        const disp       = getDisp(r.cod_interno, toNum(r.cant_bodega))
                        const umDisplay  = isCinta ? 'ROLLO' : (r.u_m || 'UNIDAD')
                        const cantBodega = Math.round(toNum(r.cant_bodega))
                        return (
                          <tr key={i} className={`border-b border-gray-50 hover:bg-amber-50 cursor-pointer transition-colors ${isTapa ? 'bg-blue-50/30' : ''}`}
                            ...
                            <td className="px-5 py-3">
                              ...
                              {isTapa  && <span className="ml-2 text-[9px] bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded-full font-medium">TAPA</span>}
                              {isCinta && <span className="ml-2 text-[9px] bg-purple-100 text-purple-600 px-1.5 py-0.5 rounded-full font-medium">CINTA</span>}
                              {isLeche && <span className="ml-2 text-[9px] bg-emerald-100 text-emerald-600 px-1.5 py-0.5 rounded-full font-medium">LECHE</span>}
                            </td>
                            <td className="px-4 py-3 text-gray-700 max-w-[220px] truncate">{r.descripcion}</td>
                            <td className="px-4 py-3 text-center text-gray-500 text-xs">{umDisplay}</td>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="U/M">
                <select value={fEntrada.u_m}
                  ...
                  {['UNIDAD','PACK','ROLLO','CAJA','KG'].map(u => <option key={u}>{u}</option>)}
                </select>
              </Field>
            </div>
            <Field label="Proveedor">
              <select value={fEntrada.proveedor}
                onChange={e => setFEntrada(p => ({...p, proveedor: e.target.value}))} className={inp}>
                <option>CENTROLAC</option>
                <option>OTRO</option>
              </select>
            </Field>
            <Field label="COD_INTERNO *">
              <select value={fEntrada.cod_interno}
                onChange={e => {
                  const prod = invMap.current[e.target.value]
                  setFEntrada(p => ({...p, cod_interno: e.target.value, descripcion: prod?.descripcion || '', u_m: prod?.u_m || 'UNIDAD'}))
                }} className={inp}>
                <option value="">— Seleccionar SKU —</option>
                {invRows.map(r => (
                  <option key={r.cod_interno} value={r.cod_interno}>{r.cod_interno} · {r.descripcion}</option>
                ))}
              </select>
            </Field>
            {fEntrada.cod_interno && (
              <div className={`rounded-lg px-3 py-2 text-xs ${SKUS_LECHE.includes(fEntrada.cod_interno) ? 'bg-amber-50 border border-amber-200 text-amber-800' : 'bg-gray-50 text-gray-600'}`}>
                {SKUS_LECHE.includes(fEntrada.cod_interno)
                  ? <><strong>⚠ Producto leche:</strong> Se descontará automáticamente 1 und de tapa ({COD_TAPA}) por cada unidad ingresada.</>
                  : <>{fEntrada.descripcion}</>}
              </div>
            )}
            <Field label="Cantidad *">
              <input type="number" min="0" value={fEntrada.cantidad}
                onChange={e => setFEntrada(p => ({...p, cantidad: e.target.value}))}
                placeholder="0" className={inp}/>
              {SKUS_LECHE.includes(fEntrada.cod_interno) && fEntrada.cantidad && (
                <p className="text-xs text-blue-600 mt-1">Tapas a descontar: <strong>{fmtN(toNum(fEntrada.cantidad))} und</strong></p>
              )}
            </Field>
          ...
          <div className="space-y-4">
            ...
            <Field label="COD_INTERNO *">
              <select value={fSalida.cod_interno}
                onChange={e => {
                  const prod = invMap.current[e.target.value]
                  setFSalida(p => ({...p, cod_interno: e.target.value, descripcion: prod?.descripcion || '', u_m: prod?.u_m || 'UNIDAD'}))
                }} className={inp}>
                <option value="">— Seleccionar SKU —</option>
                {invRows.map(r => (
                  <option key={r.cod_interno} value={r.cod_interno}>{r.cod_interno} · {r.descripcion}</option>
            ...
            {fSalida.cod_interno && (
              <div className="rounded-lg px-3 py-2 text-xs bg-purple-50 border border-purple-200 text-purple-800">
                ...
                {fmtN(getDisp(fSalida.cod_interno, toNum(invMap.current[fSalida.cod_interno]?.cant_bodega)))} {fSalida.u_m}
              </div>
            )}
            <Field label="Cantidad a retirar *">
              ...
              {fSalida.cod_interno && fSalida.cantidad && (() => {
                const disponible = getDisp(fSalida.cod_interno, toNum(invMap.current[fSalida.cod_interno]?.cant_bodega))
                const cant = toNum(fSalida.cantidad)
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="Referencia / Lote">
                <input value={pedRef} onChange={e => setPedRef(e.target.value)}
                  placeholder="PROD-2026-001" className={inp}/>
              </Field>
            </div>

            <Field label={`Seleccionar SKUs — solo leches (${pedSkus.length} seleccionados)`}>
              <div className="border border-gray-200 rounded-lg overflow-hidden max-h-72 overflow-y-auto">
                {invRows.filter(r => SKUS_LECHE.includes(r.cod_interno)).map(r => {
                  const sel   = pedSkus.includes(r.cod_interno)
                  const cant  = pedCants[r.cod_interno] || 0
                  const disp  = toNum(r.total_disponible)
                  const insuf = sel && cant > disp
                  return (
                    <div key={r.cod_interno}
                      ...
                      onClick={() => toggleSku(r.cod_interno)}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-xs font-mono font-semibold text-amber-700">{r.cod_interno}</div>
                        <div className="text-xs text-gray-600 truncate">{r.descripcion}</div>
                        <div className="text-xs text-gray-400">Disp: <span className={insuf ? 'text-red-500 font-bold' : 'text-emerald-600'}>{fmtN(disp)}</span></div>
                        {sel && cant > 0 && (
                          <div className="text-xs text-blue-600 mt-0.5">
                            → Tapas ({COD_TAPA}): <strong>{cant.toLocaleString()} und</strong>
                          </div>
                        )}
                      </div>
                      {sel && (
                        <div onClick={e => e.stopPropagation()} className="flex-shrink-0">
                          <input type="number" value={cant} step={MIN_SKU} min={MIN_SKU}
                            onChange={e => updateCant(r.cod_interno, toNum(e.target.value))}
                            className={`w-24 text-sm text-right border rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-amber-400 ${!cantValida(cant) || insuf ? 'border-red-300 bg-red-50' : 'border-gray-200'}`}/>
                          {!cantValida(cant) && cant > 0 && <div className="text-xs text-red-500 mt-0.5 text-right">múltiplo 15,800</div>}
                          {insuf && <div className="text-xs text-red-500 mt-0.5 text-right">sin stock</div>}
                        </div>
                      )}
                    </div>
                  )
            ...
            {pedSkus.length > 0 && (
              <div className="space-y-2">
                <div className={`rounded-lg px-4 py-3 text-sm flex items-center justify-between ${pedTotal >= MIN_TOTAL ? 'bg-emerald-50 border border-emerald-200' : 'bg-red-50 border border-red-200'}`}>
                  <span className="text-gray-600">Total leches:</span>
                  <span className={`font-bold text-lg ${pedTotal >= MIN_TOTAL ? 'text-emerald-700' : 'text-red-600'}`}>
                    {fmtN(pedTotal)} {pedTotal < MIN_TOTAL && <span className="text-sm font-normal">(mín {fmtN(MIN_TOTAL)})</span>}
                  </span>
                </div>
                {totalLeches > 0 && (
                  <div className="bg-purple-50 border border-purple-200 rounded-lg px-4 py-3 text-xs text-purple-800 flex items-center justify-between">
                    <span>Cinta auto ({COD_CINTA}):</span>
                    <span className="font-bold">({fmtN(totalLeches)} ÷ 18,720) × 1.42 = <strong>{Math.ceil(cintaAuto).toLocaleString()} ROLLO</strong></span>
                  </div>
                )}
              </div>
            )}
            ...
            <div className="flex gap-3 pt-2">
              <button onClick={submitPedido} disabled={saving || !pedValido}
                ...
                style={{ background: '#c8873a' }}>
                {saving ? 'Procesando...' : `Crear Pedido (${fmtN(pedTotal)} uds${totalLeches > 0 ? ` + ${Math.ceil(tapasAuto).toLocaleString()} tapas + ${Math.ceil(cintaAuto).toLocaleString()} rollos` : ''})`}
              </button>
      ...
      {showAjuste && (
        <Modal title="Ajustar Cantidad en Bodega" onClose={() => setShowAjuste(false)}>
          <div className="space-y-4">
            ...
            <div className="rounded-lg px-3 py-2 bg-blue-50 border border-blue-100 text-xs">
