/**
 * Cobertura Walmart GT — genera Excel con 4 pestañas:
 ...
 *
 * Port TypeScript de scripts/gen-cobertura-gt-v5.mjs, actualizado para usar
 * las nuevas tablas fact_inventario_walmart_* (el .mjs todavía referencia
 * las antiguas inventario_tiendas / inventario_cedi).
 */
...
// GT: WM (Walmart), DF (Despensa Familiar, mapea a "Mas X Menos" según spec),
// MD (Maxi Despensa), PZ (Paiz)
const formatoFromPunto = (punto: string): string => {
  if (punto.startsWith('GT-WM '))  return 'Walmart'
  if (punto.startsWith('GT-DF '))  return 'Mas X Menos'
  if (punto.startsWith('GT-MD '))  return 'Maxi Pali'
  if (punto.startsWith('GT-PZ'))   return 'Pali'
  return ''
}
const RPT_PREFIX: Record<string, string> = { HM: 'WM', DF: 'DF', MI: 'MD', PZ: 'PZ' }
...
export async function generarCoberturaGT(_filtros: Filtros = {}): Promise<Buffer> {
  ...
  const fechaInvRes = await pool.query(
    `SELECT MAX(fecha)::date AS f FROM fact_inventario_walmart_pdv WHERE pais='GT'`,
  )
  const fechaInvRaw = fechaInvRes.rows[0]?.f
  if (!fechaInvRaw) {
    const wb = XLSX.utils.book_new()
    const ws = XLSX.utils.aoa_to_sheet([['Sin datos en fact_inventario_walmart_pdv para GT']])
  ...
  const cediRes = await pool.query(`
    ...
    WHERE pais='GT' AND fecha = (SELECT MAX(fecha) FROM fact_inventario_walmart_cedi WHERE pais='GT')
  `)
  ...
  // ── 3. Inventario PDV GT snapshot último ────────────────────────────────
  ...
  // Última venta total por (PDV × SKU) — sin restricción 90d, solo uds > 0.
  ...
  const ultFechaRes = await pool.query(
    `SELECT MAX(fecha)::date AS f FROM fact_ventas_walmart WHERE pais='GT'`,
  )
  ...
  const cwLookup = (raw: string) => {
    ...
    return CROSSWALK_NORM[s]
        || CROSSWALK_NORM[s.slice(0, -1)]
        || null
  }
  ...
  const bRows = filas.filter(f => f.inv >= 1 && f.inv <= 5).map(f => {
    ...
    else                dohOut = Math.round(f.doh * 10) / 10
    return {
      Formato: f.formato,
      Tienda: f.punto,
      UPC: f.upc,
  ...
  // — Pestaña 4: SIN VENTAS EN 1 SEMANA
  ...
  for (const f of filas) {
    ...
    if (typeof f.cedi === 'number') a.cediTotal = f.cedi
  }

  const colsS = ['Item ID','UPC','Categoría','Descripción','Última venta GT','Días sin vender','Inv total (un)','PDVs con stock','PDVs total','CEDI (cj)','80/20','Innovación']
  ...
  for (const upc of Object.keys(aggSku)) {
    ...
    sRows.push({
      ...
      'Última venta GT': ult ?? 'Sin ventas registradas',
  ...
  wb.Props = {
    Title:   `Cobertura Walmart GT — ${fechaHuman}`,
